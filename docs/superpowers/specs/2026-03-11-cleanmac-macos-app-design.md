...
| 菜单项 | Action |
|--------|--------|
| 退出 | `NSApp.terminate(nil)` (closes popover first) |

**实现方式**: 使用 `button.sendAction(on: [.leftMouseUp, .rightMouseUp])` 捕获所有点击事件，在 `togglePopover()` 中通过 `NSApp.currentEvent?.type == .rightMouseUp` 检测右键并调用 `showRightClickMenu()`。
...
├── AppDelegate.swift           # NSStatusItem, NSPopover, icon rotation, right-click menu
...
- **Right-click via sendActionOn**: Single handler `togglePopover()` receives all clicks; right-click detected via `NSApp.currentEvent?.type == .rightMouseUp`, avoiding subclassing `NSStatusItem.button`.
